...
class LinearNN(nn.Module):
	"""
	Defines a linear neural network (single layer linear network)
	Args:
		inp_size: Input dimensionality
		out_size: Output dimensionality, default 1
	"""
...
class MultiLayerNN(nn.Module):
	"""
	Defines a linear/non-linear multi-layer neural network (fully connected)
	Args:
		inp_size: Input dimensionality
		num_layers: Number of hidden layers, default 1
		hidden_dim: Dimensionality of hidden layers, default None --> set to inp_size
		non_linearity: Non linearity to be applied in each hidden layer, default None
		out_size: Output dimensionality, default 1
	"""
